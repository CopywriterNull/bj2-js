...
var productIdsToFetch = window.SHOP_CONFIG
				.map(function (item) {
        ...
        ... on Product {
					...
					if (domEl) {
						if (!!product?.options) {
							...
							product.options.forEach(function (option) {
								...
								pathElement.setAttribute('d', 'M10.3003 5.59999L7.50029 8.39999L4.70029 5.59999');
								...
								if (swatchPng) {
									elTitleSwatch.innerHTML = elShopImage(swatchPng.reference.image.originalSrc, value, 32, 32);
								}
								if (swatchColor) {
								...
								option.values.forEach(function (value, index) {
									...
									if (!!selectedVariant?.selectedOptions?.length) {
										var currentValue = selectedVariant.selectedOptions.find(function (item) {
									...
									if ((option.name === 'Color') || (option.name === 'Colour')) {
										...
										if (swatchPng) {
											elValue.innerHTML = elShopImage(swatchPng.reference.image.originalSrc, value, 32, 32);
										}
										else if (swatchColor) {
									...
									elValue.onclick = function (e) {
										...
										body.style.overflow = 'unset';
										// Update option title
										var elTitleValue = e.target.closest('.shop-custom-option')?.querySelector('.shop-custom-option__title-value');
			...
			function currentVariantHandler(product, el) {
				var arrOptions = product.options.map(function (option) {
					return el.querySelector('.shop-custom-options').dataset[option.name.toLowerCase()];
				});
				...
				if (domGalleryEl) {
					...
					if (hasColor) {
						domGalleryEl.innerHTML = elShopImage(currentVariant?.image?.src, currentVariant?.image?.altText || currentVariant.title, 300, 300);
					}
				}
			...
			document.querySelectorAll('.shop-custom-quantity__plus').forEach(function (btn) {
				...
				btn.addEventListener('click', function () {
					...
					var elPriceDiscount = elShop.querySelector('.shop-custom-submit__price-compare');
       ...
       `;
          ...
          <div class="swiper ${elId}-mySwiper2">
             ...
             ${images.map(function (img) {
					return `<div class="swiper-slide">${elShopImage(img.src, img.alt, 530, 530)}</div>`;
				}).join('')}
             </div>
          </div>
          <div class="swiper ${elId}-mySwiper">
             <div class="swiper-wrapper">
                ${images.map(function (img) {
					return `<div class="swiper-slide">${elShopImage(img.src, img.alt, 75, 75)}</div>`;
				}).join('')}
             </div>
          </div>
       `;
