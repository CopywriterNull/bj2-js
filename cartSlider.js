			...
			var vm = new Vue( {
				...
				methods: {
					...
					cartItemQuantityIncrease( id ) {
						var elementPos = this.cartData.map( function ( x ) { return +x.id; } )
							.indexOf( +id );
					...
					domToggleCartDrawer() {
						...
						if ( cartTrigger ) {
							document.querySelector( classCartTrigger )
					...
					domAddToCart() {
						...
						if ( atcButtonElementAll && !!atcButtonElementAll.length ) {
							atcButtonElementAll.forEach( function ( atcButton ) {
					...
					onCheckout() {
						...
						this.cartData.map( function ( item, index ) {
					...
					cartUpsellHandler( data ) {
							...
							var cartUpsellTitle = cartUpsellWrapper.querySelector( '.cart-upsell__title' );

							if ( cartUpsellEl && cartUpsellItemsEl ) {
								cartUpsellItemsEl.forEach( function ( item ) {
									var elProductId = item.dataset.upsellProductId;
									var findUpsellItemInData = data.some( function ( el ) { return +el.idProduct === +elProductId; } );
				...
				watch: {
					cartData( data ) {
						localStorage.setItem( LOCAL_STORAGE_CART_DATA, JSON.stringify(
							{
								items: data
							}
						) );
